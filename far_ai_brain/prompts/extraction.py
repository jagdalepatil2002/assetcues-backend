...
3. If serial numbers are visible ANYWHERE on the document (including continuation pages, annexures, or a separate "Serial Number" column that appears on a later page), assign them to individual assets in order. Serial numbers often appear on page 2+ as a continuation of the line items table. Scan ALL pages before concluding no serials exist. If multiple serial numbers appear on one line separated by "/" (e.g. "SN001/SN002/SN003"), treat each "/" as a delimiter and assign exactly one serial number to each individual asset unit in order.
...
9. SERIAL NUMBERS: Scan ALL pages of the document for serial/IMEI numbers. They may appear on a later page as a "Serial Number" column continuation of the line items table. Capture them in serial_numbers_listed as an array of strings. CRITICAL: If multiple serials appear on one line separated by "/" (e.g. "PW0N9Y00/PW0N9Y01/PW0N9XZX"), split them into separate array elements — never put all serials as a single joined string.
